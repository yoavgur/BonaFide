"""The Explanation container class.

Adapted from CC-SHAP shap/_explanation.py with imports fixed for shap_fork.
"""
...
class Explanation(metaclass=MetaExplanation):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        if Slicer is None:
            # Fallback: store attributes directly when slicer is not available
            self._values = values
            ...
            self._hierarchical_values = hierarchical_values
            self._clustering = clustering
            self._s = None
            return

        if len(_compute_shape(feature_names)) == 1:
        ...
        if len(_compute_shape(output_names)) == 1:
    ...
    def shape(self):
        ...
        return _compute_shape(self._values)
    ...
    def base_values(self, new_base_values):
        if self._s is not None:
            ...
            self._base_values = new_base_values
    ...
    def display_data(self, new_display_data):
        ...
        if self._s is not None:
            ...
            self._display_data = new_display_data
    ...
    def output_names(self, new_output_names):
        if self._s is not None:
            ...
            self._output_names = new_output_names
    ...
    def main_effects(self, new_main_effects):
        if self._s is not None:
            ...
            self._main_effects = new_main_effects

    @property
    def hierarchical_values(self):
        if self._s is not None:
            return self._s.hierarchical_values
        return self._hierarchical_values
    ...
    def hierarchical_values(self, new_hierarchical_values):
        if self._s is not None:
            ...
            self._hierarchical_values = new_hierarchical_values
    ...
    def clustering(self, new_clustering):
        if self._s is not None:
            ...
            self._clustering = new_clustering
    ...
    def __getitem__(self, item):
        ...
        for t in item:
            ...
            if issubclass(type(t), OpChain):
                t = t.apply(self)
                if issubclass(type(t), (np.int64, np.int32)):
                    t = int(t)
                elif issubclass(type(t), np.ndarray):
                    t = [int(v) for v in t]
            elif issubclass(type(t), Explanation):
                t = t.values
            elif isinstance(t, str) and self._s is not None:
                output_names_dims = []
                ...
                feature_names_dims = []
                if self._s is not None and "feature_names" in self._s._objects:
                ...
                if (
                    ...
                    for i, val_i in enumerate(self.values):
                        for s, v, d in zip(
                            self.feature_names[i], val_i, self.data[i]
                        ):
                            if s == t:
                                new_values.append(v)
                                new_data.append(d)
    ...
    def _apply_binary_operator(self, other, binary_op, op_name):
        ...
        if isinstance(other, Explanation):
            ...
            if new_exp.base_values is not None:
                new_exp.base_values = binary_op(
                    new_exp.base_values, other.base_values
                )
        else:
    ...
    def _numpy_func(self, fname, **kwargs):
        ...
        if axis == 0:
            ...
            new_self = new_self[2]
        if axis in [0, 1, 2]:
            new_self.op_history = new_self.op_history[:-1]
        ...
        if (
            ...
            if new_self.data is not None:
                try:
                    new_self.data = getattr(np, fname)(
                        np.array(self.data), **kwargs
                    )
                except Exception:
                    new_self.data = None
            if (
                ...
                new_self.base_values = getattr(np, fname)(
...
def group_features(shap_values, feature_map):
    ...
    for name in feature_map:
        reverse_map[feature_map[name]] = reverse_map.get(feature_map[name], []) + [
            name
        ]
...
def compute_output_dims(values, base_values, data, output_names):
    ...
    output_dims = range(
        len(data_shape) + interaction_order, len(values_shape)
    )
    ...
    if not hasattr(x, "__len__") or isinstance(x, str):
        ...
        if len(x) == 0:
            ...
            if first_shape == tuple():
                ...
                return (len(x),) + tuple(
                    first_shape[j] if match else None
...
def list_wrap(x):
    """A helper to patch things since slicer doesn't handle arrays of arrays."""
