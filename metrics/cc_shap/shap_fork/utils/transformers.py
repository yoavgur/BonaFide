"""Transformer model helpers for shap_fork.

Adapted from CC-SHAP shap/utils/transformers.py with TF models stripped
and newer model classes added.
"""
...
MODELS_FOR_CAUSAL_LM = [
    ...
    "transformers.CodeLlamaForCausalLM",
]
...
def is_transformers_lm(model):
    ...
    return safe_isinstance(model, "transformers.PreTrainedModel") and \
    ...
    if len(null_tokens) == 1:
        ...
        if hasattr(tokenizer, "special_tokens_map") and hasattr(tokenizer, "decode"):
            ...
            if safe_isinstance(tokenizer, "transformers.models.llama.tokenization_llama.LlamaTokenizer"):
                # the LLaMA tokenizer is a little weirder than the shap library originally anticipated
                keep_prefix = 0
                keep_suffix = 0
