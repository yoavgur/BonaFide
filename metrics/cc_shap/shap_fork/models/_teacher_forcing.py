...
class TeacherForcing(Model):
    ...
    def model_inference(self, inputs, output_ids):
        """ This function performs model inference for pytorch models.
        ...
        if self.similarity_model_type == "pt":
            ...
            with torch.no_grad():
                if self.similarity_model.config.is_encoder_decoder:
                    ...
                    if not safe_isinstance(self.similarity_model, ["transformers.BloomForCausalLM", "transformers.OPTForCausalLM"]):
            ...
            raise ValueError(f"Unsupported similarity_model_type: {self.similarity_model_type}. Only 'pt' (PyTorch) is supported.")
        return logits
